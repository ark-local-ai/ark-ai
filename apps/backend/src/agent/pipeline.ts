...
async function editor(
  prompt: string, kind: string, sections: StepContent[],
): Promise<{ sections: StepContent[]; viaLLM: boolean }> {
...
/** 多 Agent 内容管线：分析 → 写 → 校验，返回最终 sections */
export async function runContentPipeline(
  ...
  // 分析师
  const { points, viaLLM: analystLLM } = await analyst(prompt, plan, kind);
  // 写手（依赖分析师要点）
  const { sections, viaLLM: writerLLM } = await writer(prompt, plan, kind, points);
  // 校验
  const { sections: finalSections, viaLLM: editorLLM } = await editor(prompt, kind, sections);
