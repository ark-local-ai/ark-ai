...
function buildContextText(ctx: Pick<PipelineContext, "web" | "knowledge">): string {
  const parts: string[] = [];
...
export async function gatherContext(prompt: string, userId?: string): Promise<PipelineContext> {
  ...
  const contextText = buildContextText({ web, knowledge });
  const hasRef = web.some((w) => !w.error) || knowledge.length > 0;
  return { web, knowledge, contextText, hasRef };
}

/** 把参考资料追加为一个 section（有引用才加）。 */
export function appendRefSection(
  sections: { title: string; paragraphs: string[] }[],
  ctx: Pick<PipelineContext, "web" | "knowledge">,
